...
class _WorkoutScreenState extends State<WorkoutScreen> {
  ...
  void initState() { 
    super.initState(); 
    _loadInitialData(); 
    // ✅ UI 렌더링 후 즉시 삼성 최적화 권한 요청 (묶음 요청)
    WidgetsBinding.instance.addPostFrameCallback((_) => _requestBlePermissions());
  }

  // ✅ 수정 포인트: 삼성 기기 최적화 권한 요청 로직
  Future<void> _requestBlePermissions() async {
    await [
      Permission.bluetoothScan,
      Permission.bluetoothConnect,
      Permission.locationWhenInUse, // 사용자님 분석대로 WhenInUse 사용
    ].request();
  }
  ...
  // ✅ 수정 포인트: 스캔 안정성 강화 (어댑터 체크 및 위치 권한 재확인)
      ...
      builder: (context) => StatefulBuilder(builder: (context, setModalState) {
        ...
        return Container(padding: const EdgeInsets.all(20), height: MediaQuery.of(context).size.height * 0.4, child: Column(children: [
          ...
          Expanded(child: _filteredResults.isEmpty ? const Center(child: CircularProgressIndicator(color: Colors.greenAccent)) : ListView.builder(itemCount: _filteredResults.length, itemBuilder: (context, index) {
            ...
            return ListTile(leading: const Icon(Icons.watch, color: Colors.blueAccent), title: Text(r.device.platformName), onTap: () { Navigator.pop(context); _connectToDevice(r.device); });
          })) 
        ]));
...
class _HistoryScreenState extends State<HistoryScreen> {
    ...
    showModalBottomSheet(context: context, backgroundColor: Colors.white, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(25))), 
      builder: (context) => Container(height: 350, padding: const EdgeInsets.fromLTRB(20, 15, 20, 20), child: Column(children: [
        ...
        Expanded(child: BarChart(BarChartData(
          ...
          barTouchData: BarTouchData(enabled: false, touchTooltipData: BarTouchTooltipData(tooltipBgColor: Colors.transparent, tooltipMargin: 4, getTooltipItem: (group, groupIndex, rod, rodIndex) => BarTooltipItem("${rod.toY.toStringAsFixed(1)}", TextStyle(color: color.withOpacity(0.9), fontWeight: FontWeight.bold, fontSize: 11)))),
          gridData: const FlGridData(show: false), titlesData: const FlTitlesData(show: false), borderData: FlBorderData(show: false), 
        ))),
        const SizedBox(height: 10),
        const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.bolt, size: 12, color: Colors.orange), Text(" 배경 구간: 소모 칼로리별 운동 강도 가이드", style: TextStyle(fontSize: 10, color: Colors.grey))])
      ]))
    );
  } 
  ...
  Widget build(BuildContext context) {
    ...
    return Theme(data: ThemeData(brightness: Brightness.light), child: Scaffold(backgroundColor: const Color(0xFFF1F5F9), appBar: AppBar(title: const Text("기록 리포트"), backgroundColor: Colors.white, foregroundColor: Colors.black, elevation: 0), body: SingleChildScrollView(child: Column(children: [
