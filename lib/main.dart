...
class _WorkoutScreenState extends State<WorkoutScreen> {
  ...
  double _calories = 0.0, _goalCalories = 150.0; // 사진 1번 기준 150kcal
  ...
  void _showDeviceScanPopup() async {
    ...
    showModalBottomSheet(
      ...
      builder: (context) => StatefulBuilder(builder: (context, setModalState) {
        ...
        return Container(padding: const EdgeInsets.all(20), height: MediaQuery.of(context).size.height * 0.4, child: Column(children: [const Text("워치 검색", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), const SizedBox(height: 10), Expanded(child: ListView.builder(itemCount: _filteredResults.length, itemBuilder: (context, index) => ListTile(title: Text(_filteredResults[index].device.platformName), trailing: const Icon(Icons.bluetooth_searching, color: Colors.blueAccent), onTap: () { Navigator.pop(context); _connectToDevice(_filteredResults[index].device); } )))]));
            ...
            child: Column(children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                ...
                ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(backgroundColor: Colors.black45, foregroundColor: Colors.greenAccent, side: const BorderSide(color: Colors.greenAccent)),
                  child: Text(_isWatchConnected ? "연결됨" : "워치 연결"),
                )
              ]),
              const Spacer(),
              // ✅ 중앙 자전거 네온 아이콘 (1번 사진)
              Image.asset('assets/icon/bike_ui_dark.png', height: 250),
              const Spacer(),
              // ✅ 목표 칼로리 바 (1번 사진 스타일)
              Container(
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
                _iconBtn(_isWorkingOut ? Icons.pause : Icons.play_arrow, "시작", () {
                ...
                _iconBtn(Icons.refresh, "리셋", () { setState(() { _calories = 0; _duration = Duration.zero; _hrSpots.clear(); }); }),
                _iconBtn(Icons.save, "저장", () async {
                ...
                _iconBtn(Icons.calendar_month, "기록", () => Navigator.push(context, MaterialPageRoute(builder: (c) => HistoryScreen(records: _records)))),
              ]),
              const SizedBox(height: 10),
            ]),
          ),
  ...
  Widget _iconBtn(IconData i, String l, VoidCallback t) => Column(children: [
    ElevatedButton(onPressed: t, style: ElevatedButton.styleFrom(padding: const EdgeInsets.all(15), backgroundColor: Colors.white10, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15))), child: Icon(i, size: 30, color: Colors.white)),
...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Theme(
      ...
      child: Scaffold(
        appBar: AppBar(title: const Text("기록 리포트"), centerTitle: true),
        body: Column(children: [
          ...
          const SizedBox(height: 20),
          // ✅ 빌드 에러 해결: getTooltipColor -> tooltipBgColor로 수정 (또는 제거)
          SizedBox(
            height: 120,
            child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 20),
              child: BarChart(BarChartData(
                barTouchData: BarTouchData(touchTooltipData: BarTouchTooltipData()), // 에러 방지를 위해 기본값 사용
                gridData: const FlGridData(show: false), titlesData: const FlTitlesData(show: false), borderData: FlBorderData(show: false),
                barGroups: List.generate(daily.length, (i) => BarChartGroupData(x: i, barRods: [BarChartRodData(toY: daily[i].calories, color: Colors.blueAccent, width: 15)]))
              )),
            ),
          ),
          Expanded(child: ListView.builder(itemCount: daily.length, itemBuilder: (c, i) => ListTile(
            leading: const Icon(Icons.directions_bike, color: Colors.blueAccent),
            title: Text("${daily[i].calories.toInt()} kcal 소모"),
            subtitle: Text("${daily[i].duration.inMinutes}분 / ${daily[i].avgHR} bpm"),
          )))
        ]),
      ),
    );
