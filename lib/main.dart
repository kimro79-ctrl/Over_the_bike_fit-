...
class _WorkoutScreenState extends State<WorkoutScreen> {
          ...
          child: Padding(
            ...
            child: Column(children: [
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
                ...
                _iconBtn(Icons.refresh, "RESET", Colors.white70, () { setState(() { _calories = 0; _duration = Duration.zero; _hrSpots.clear(); }); }),
  ...
  Widget _stat(String l, String v, Color c) => Column(children: [Text(l, style: const TextStyle(fontSize: 12, color: Colors.white60)), const SizedBox(height: 5), Text(v, style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: c))]);
    ...
    return Theme(
      ...
      child: Scaffold(
        ...
        body: Column(children: [
          TableCalendar(locale: 'ko_KR', firstDay: DateTime(2024), lastDay: DateTime(2030), focusedDay: _selectedDay, selectedDayPredicate: (day) => isSameDay(_selectedDay, day), onDaySelected: (sel, foc) => setState(() => _selectedDay = sel), calendarFormat: CalendarFormat.twoWeeks),
          const SizedBox(height: 20),
          SizedBox(height: 150, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: BarChart(BarChartData(barTouchData: BarTouchData(touchTooltipData: BarTouchTooltipData()), gridData: const FlGridData(show: false), titlesData: const FlTitlesData(show: false), borderData: FlBorderData(show: false), barGroups: List.generate(daily.length, (i) => BarChartGroupData(x: i, barRods: [BarChartRodData(toY: daily[i].calories, color: Colors.blueAccent, width: 16)])))))),
          Expanded(child: ListView.builder(itemCount: daily.length, itemBuilder: (c, i) => ListTile(leading: const Icon(Icons.directions_bike, color: Colors.blueAccent), title: Text("${daily[i].calories.toInt()} kcal 소모"), subtitle: Text("${daily[i].duration.inMinutes}분 운동 / 평균 ${daily[i].avgHR} BPM"))))
        ]),
      ),
    );
  }
