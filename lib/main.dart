...
import 'package:table_calendar/table_calendar.dart'; // 달력 라이브러리
...
class _AssetSplashScreenState extends State<AssetSplashScreen> {
  ...
  @override
  Widget build(BuildContext context) {
    return const Scaffold(body: Center(child: Text("BIKE FIT", style: TextStyle(fontSize: 32, fontWeight: FontWeight.bold))));
  }
}
...
class _WorkoutScreenState extends State<WorkoutScreen> {
  ...
  Future<void> _saveWorkout() async {
    if (_duration.inSeconds < 10) return; // 10초 미만 운동은 저장 안 함
  ...
  void _showDeviceScanPopup() async {
    ...
    if (await FlutterBluePlus.isOn == false) { _showToast("블루투스를 켜주세요"); return; }
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setModalState) {
          _scanSubscription = FlutterBluePlus.onScanResults.listen((results) {
            if (mounted) {
              setModalState(() {
                // 이름이 있는 모든 기기 검색 (어메이즈핏 포함)
                _filteredResults = results.where((r) => (r.device.platformName.isNotEmpty || r.advertisementData.advName.isNotEmpty)).toList();
              });
            }
          });
          return Container(
            ...
            child: Column(children: [
              const Text("워치 검색", style: TextStyle(fontWeight: FontWeight.bold)),
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ListTile(
                            ...
                            title: Text(name),
                            onTap: () { Navigator.pop(context); _connectToDevice(r.device); },
                          );
                        },
                      ),
              )
      ...
      if (_isWorkingOut) {
        _workoutTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
          setState(() {
            ...
            if (_heartRate > 0) _calories += 0.12;
          });
        });
      } else {
        _workoutTimer?.cancel();
        _saveWorkout(); // ✅ 중단 시 자동 저장
      }
    });
  }
  ...
  Widget _dataRow(String l, String v) => Padding(
    ...
    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(l), Text(v, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold))]),
  );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          TextButton(onPressed: _showDeviceScanPopup, child: Text(_isWatchConnected ? "연결됨" : "워치 연결", style: const TextStyle(color: Colors.greenAccent))),
          IconButton(icon: const Icon(Icons.history), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (c) => HistoryScreen(records: _records)))),
        ],
      ),
      body: Column(
        children: [
          Expanded(child: Padding(padding: const EdgeInsets.all(20), child: LineChart(LineChartData(titlesData: const FlTitlesData(show: false), borderData: FlBorderData(show: false), gridData: const FlGridData(show: false), lineBarsData: [LineChartBarData(spots: _hrSpots.isEmpty ? [const FlSpot(0, 0)] : _hrSpots, isCurved: true, dotData: const FlDotData(show: false), color: Colors.greenAccent)])))),
          _dataRow("심박수", "$_heartRate BPM"),
          _dataRow("평균", "$_avgHeartRate BPM"),
          _dataRow("칼로리", "${_calories.toStringAsFixed(1)} kcal"),
          _dataRow("시간", "${_duration.inMinutes}:${(_duration.inSeconds % 60).toString().padLeft(2, '0')}"),
          const SizedBox(height: 20),
          Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: LinearProgressIndicator(value: progress, minHeight: 10, color: Colors.greenAccent)),
          const SizedBox(height: 40),
          ElevatedButton(onPressed: _toggleWorkout, style: ElevatedButton.styleFrom(backgroundColor: _isWorkingOut ? Colors.red : Colors.green, foregroundColor: Colors.white), child: Text(_isWorkingOut ? "운동 종료 및 저장" : "운동 시작")),
          const SizedBox(height: 40),
        ],
      ),
    );
...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final dailyRecords = widget.records.where((r) {
      ...
      return r.date == DateFormat('yyyy-MM-dd').format(_selectedDay!);
    }).toList();
            ...
            child: dailyRecords.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return ListTile(
                        leading: const Icon(Icons.directions_bike, color: Colors.greenAccent),
                        title: Text("${r.calories.toInt()} kcal 소모"),
                        subtitle: Text("평균 ${r.avgHR} BPM / ${r.duration.inMinutes}분 운동"),
                      );
                    },
                  ),
          ),
